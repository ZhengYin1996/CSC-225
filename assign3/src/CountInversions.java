...
public class CountInversions{
	...
	*/
	public static int CountInversions(int[] A){
		...
		int times=0;
		int j=0;
		while(j<counter.size()-1)
		{
			if((int)counter.get(j)==0)
			{
				j++;
			}
			else{
				int pos=j;
				while(pos<(counter.size()-1) && (int)counter.get(pos)>0)
				{
					//swap the elements.
					if((int)counter.get(pos)>(int)counter.get(pos+1))
					{
						Collections.swap(counter,pos,pos+1);
						int temp = (int)counter.get(pos)+1;
						counter.remove(pos);
						counter.add(pos,temp);
						temp = (int)counter.get(pos+1)-1;
						counter.remove(pos+1);
						counter.add(pos+1,temp);
						count++;
					}
